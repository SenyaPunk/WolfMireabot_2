...
async def my_army_cmd(message: Message):
    ...
    upgrades = army_manager.get_army_upgrades(army_manager.get_user_army_key(user_id))
    tech_line = (
        f"🔬 <b>Модернизация:</b> 🛸 {upgrades.get('drones',0)}/5 | 🏰 {upgrades.get('fortress',0)}/5 | "
        f"🏥 {upgrades.get('medicine',0)}/5 | 📡 {upgrades.get('ewar',0)}/5 | 🚚 {upgrades.get('logistics',0)}/5 (/улучшения)"
    )
    ...
    if member_info.get("rank") == RANK_CREATOR:
        msg_text += (
            ...
            "• <code>/улучшения</code> — меню модернизации (дроны, база, мед, рэб)\n"
            "• <code>/прокачать [ветка]</code> — улучшить технологии за счёт казны\n"
...
@router.message(Command("upgrades", "улучшения", "апгрейды", "технологии", "модернизация"))
async def army_upgrades_cmd(message: Message):
    """Просмотр меню улучшений и военных технологий армии."""
    user_id = message.from_user.id
    army_key = army_manager.get_user_army_key(user_id)
    if not army_key:
        await message.reply(
            "🪖 <b>Вы не состоите ни в одной армии.</b>\n\n"
            "Вступите в существующую армию или создайте свою: /армия",
            parse_mode="HTML"
        )
        return

    menu_text = army_manager.format_upgrades_menu(army_key)
    await message.reply(menu_text, parse_mode="HTML", disable_web_page_preview=True)


@router.message(Command("upgrade", "прокачать", "улучшить", "апгрейд"))
async def upgrade_branch_cmd(message: Message):
    """Прокачка выбранной ветки технологий Главкомом за счёт казны."""
    user_id = message.from_user.id
    parts = message.text.split()[1:]
    if not parts:
        await message.reply(
            "⚠️ <b>Использование:</b> <code>/прокачать [ветка]</code>\n\n"
            "💡 <i>Доступные направления:</i>\n"
            "• <code>/прокачать дрон</code> — дронопарк и FPV-комплексы\n"
            "• <code>/прокачать база</code> — укрепрайон (держит рубеж без штурмовиков!)\n"
            "• <code>/прокачать мед</code> — полевой госпиталь и здоровье бойцов\n"
            "• <code>/прокачать рэб</code> — системы РЭБ и защита от дронов\n"
            "• <code>/прокачать снабжение</code> — логистика и увеличенные трофеи\n\n"
            "📋 Посмотреть все ветки и цены: <code>/улучшения</code>",
            parse_mode="HTML"
        )
        return

    branch_query = parts[0].lower()
    success, result_msg, _ = army_manager.upgrade_army_branch(user_id, branch_query)
    await message.reply(result_msg, parse_mode="HTML")
...
async def army_help_cmd(message: Message):
    ...
    help_text = (
        ...
        "👑 <b>Главнокомандующий</b> — создатель армии. Управляет мобилизацией, казной, модернизацией, боевыми приказами, объявляет СВО или капитуляцию.\n"
        "⚔️ <b>Штурмовик</b> — мобилизованный боец на передке. Находится на линии соприкосновения. Штурмует вражеские позиции, запускает дроны и берет пленных!\n"
        "🎖️ <b>Рядовой (Тыловой резерв)</b> — тыловое обеспечение. Не рискует попасть в плен, обеспечивает медпомощь, подвоз БК, наносит удары дронами и ремонтирует укрепрайон.\n"
        "⛓️ <b>Военнопленный</b> — захваченный враг. Находится в застенках победителя до выкупа или помилования.\n\n"

        "<b>3️⃣ МОДЕРНИЗАЦИЯ И УЛУЧШЕНИЯ АРМИИ:</b>\n"
        "• Главком может прокачивать 5 веток технологий за монеты казны:\n"
        "  <code>/улучшения</code> — меню всех веток модернизации\n"
        "  <code>/прокачать [дрон | база | мед | рэб | снабжение]</code>\n"
        "• 🛸 <b>Дронопарк (ур. 1-5):</b> открывает команду <code>/дрон</code> для ударов БПЛА по врагу или вражеской базе (+10..35% урона армии).\n"
        "• 🏰 <b>Укрепрайон (ур. 1-5):</b> <b>позволяет армии держаться в войне даже без штурмовиков!</b> Даёт 150..1100 HP базе, турели отстреливают врагов, тыл может чинить базу (<code>/укрепить</code>).\n"
        "• 🏥 <b>Госпиталь (ур. 1-5):</b> увеличивает здоровье бойцов (+10..50 Max HP), лечит раненых, даёт шанс спасброска от гибели/плена.\n"
        "• 📡 <b>РЭБ (ур. 1-5):</b> сбивает и перехватывает дроны врага, открывает команду <code>/рэб</code> (глушение частот противника).\n"
        "• 🚚 <b>Логистика (ур. 1-5):</b> мощный подвоз снарядов, +15..30% к захватываемым трофеям из казны побеждённых!\n\n"

        "<b>4️⃣ МОБИЛИЗАЦИЯ И РОТАЦИЯ:</b>\n"
        ...
        "<b>5️⃣ СПЕЦИАЛЬНАЯ ВОЕННАЯ ОПЕРАЦИЯ (СВО):</b>\n"
        ...
        "  — <code>/штурм</code> (кд 30с) — накат на штурмовиков врага или на вражеский укрепрайон (если бойцов нет)!\n"
        "  — <code>/оборона</code> (кд 45с) — окопаться на 1.5 мин (-50% входящего урона).\n"
        "  — <code>/дрон</code> (кд 40с) — запуск FPV-дрона по живой силе или укреплениям.\n"
        "• <b>Действия тыла (рядовых):</b>\n"
        "  — <code>/медпомощь</code> (кд 25с) — перевязать раненого бойца, снимает ранение.\n"
        "  — <code>/снабжение</code> (кд 45с) — доставить БК (+30..40% урона).\n"
        "  — <code>/дрон</code> (кд 40с) — нанести авиаудар БПЛА из тыла.\n"
        "  — <code>/укрепить</code> (кд 45с) — ремонт и укрепление рубежа обороны (+HP базы).\n"
        "• <b>Действия Главкома:</b>\n"
        "  — <code>/приказ [атака|оборона]</code> — тактическое распоряжение.\n"
        "  — <code>/рэб</code> — включить глушение радиосвязи противника (РЭБ-4+).\n"
        "  — <code>/капитуляция</code> — признать поражение.\n\n"

        "<b>6️⃣ «ТУМАН ВОЙНЫ» И БАЛАНСИРОВКА:</b>\n"
        "• 🌦️ <b>Погода:</b> каждые 3 минуты меняются условия (туман, дождь, шквал снижают точность и урон).\n"
        "• 📦 <b>Ленд-лиз:</b> аутсайдер получает внешнюю гуманитарную помощь (+30 HP всем бойцам) либо партизаны взрывают тыл лидера!\n"
        "• 📡 <b>РЭБ «Красуха»:</b> на 60 секунд глушит связь тыла (нельзя лечить и наводить дроны).\n\n"

        "<b>7️⃣ КАЗНА И ВОЕННОПЛЕННЫЕ:</b>\n"
        "• <code>/казна</code> — бюджет армии, трофеи и победы.\n"
        "• <code>/пополнить_казну [сумма]</code> — внести монеты в бюджет (деньги идут на улучшения!).\n"
        "• <code>/военнопленные</code> — застенки и захваченные штурмовики противника.\n"
        "• <code>/выкуп_пленного [ID] [сумма]</code> — выкупить соратника из чужого плена.\n"
