...
def render_poker_table_image(game_state: Dict[str, Any], is_showdown: bool = False) -> io.BytesIO:
    ...
    # Небольшой едва заметный водяной знак логотипа Волки МИРЭА (смещен чуть ниже центра)
    ...
    if logo_path.exists():
        try:
            ...
            logo.putalpha(alpha)
            img.paste(logo, ((w - logo_size) // 2, 265), mask=logo)
        except Exception:
            pass

    # Брендинг университета и чата на сукне (мягкий и ненавязчивый)
    f_brand = get_font(12, bold=False)
    draw.text((w // 2, 400), "РТУ МИРЭА • ВОЛКИ", fill=(26, 100, 66), font=f_brand, anchor="mm")
